...
$(document).keydown(function(e) {
    ...
    return {
        ...
        link: function($scope, $element) {
            $scope.active = !1, $element.bind("click", function() {
                if (layers.isElementInLayers(this) || !(layers.getLayersList.length > 1)) {
                    var element = angular.element(this), scope = element.scope();
                    layers.updateLayers(this), scope.active || (scope.active = !0), scope.active && layers.getLayersList.indexOf(this) < 0 && layers.getLayersList.push(this), 
                    element[0].offsetWidth < 50 && ($scope.smallWrap = !0), scope.$apply();
    ...
    function updateLayers(element) {
        !isElementInLayers(element) && _layers.length > 0 && popLastLayer();
    }
    function isElementInLayers(element) {
        return _layers.indexOf(angular.element(element)[0]) > -1 ? !0 : "HTML" !== angular.element(element).parent()[0].tagName ? isElementInLayers(angular.element(element).parent()) : !1;
    }
    function popLastLayer() {
        var $topLayerScope = angular.element(_layers[_layers.length - 1]).scope();
        $topLayerScope.active = !1, $topLayerScope.$apply(), _layers.pop();
    }
    var _layers = [];
