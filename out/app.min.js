...
$(document).keydown(function(e) {
    ...
    $("#in-guidelines").remove()) : (agGridState = !0, $("BODY").append('<div id="in-guidelines"><div class="wrapper" style="height: 100%"><div id="guidelines"><div class="for-guidelines"><div class="col-edge-1"><div class="box"><p class="grid-p"></p></div></div><div class="col-1"><div class="box"><p class="grid-p"></p></div></div><div class="col-1"><div class="box"><p class="grid-p"></p></div></div><div class="col-1"><div class="box"><p class="grid-p"></p></div></div><div class="col-1"><div class="box"><p class="grid-p"></p></div></div><div class="col-edge-1"><div class="box"><p class="grid-p"></p></div></div><div class="left-padding-grid"></div><div class="right-padding-grid"></div></div></div></div></div>')));
    ...
    return {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            var inputDay = element.find("input:eq(0)"), inputMonth = element.find("input:eq(1)"), inputYear = element.find("input:eq(2)"), inputs = element.find("input");
            scope.groupCtrl = attrs.groupCtrl, ngModelCtrl.$render = function() {
        ...
        template: '<div class="dib" data-split-fields><input class="t-input t-input__micro" data-ng-class="{ first: !groupCtrl }" data-modifier=\'["RegExpFilter:[^\\\\d]"]\' autocomplete="off" placeholder="ДД" maxlength="2"><input class="t-input t-input__micro" data-modifier=\'["RegExpFilter:[^\\\\d]"]\' autocomplete="off" placeholder="ММ" maxlength="2"><input class="t-input t-input__mini last" data-modifier=\'["RegExpFilter:[^\\\\d]"]\' autocomplete="off" placeholder="ГГГГ" maxlength="4"></div>',
        replace: !0
    };
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            $scope.visible = !1, $scope.caller = $sce.trustAsHtml($attrs.caller), $scope.toggleDropdown = function() {
                0 === layers.layersList.length ? ($scope.visible = !0, layers.layersList.push($element[0])) : 1 != layers.layersList.length || layers.isUpInTree($element[0]) ? layers.layersList.length > 0 && layers.isUpInTree($element[0]) && layers.layersList.indexOf($element[0]) < 0 ? ($scope.visible = !0, 
                ...
                layers.layersList.pop()) : (angular.element(layers.layersList[0]).scope().visible = !1, 
                layers.layersList.pop(), $scope.visible = !0, layers.layersList.push($element[0])), 
                $element[0].offsetWidth < 50 && ($scope.smallWrap = !0);
            }, $scope.$watch("visible", function(isVisible) {
                var doc = angular.element(document).find("BODY")[0];
                angular.forEach($element.children(), function(element) {
                    angular.element(element).hasClass("dropdown_container") && (element.style.display = isVisible ? "block" : "none", 
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            $scope.$watch("visible", function(isVisible) {
                isVisible && "function" == typeof $scope[$attrs.onOpen] && $scope[$attrs.onOpen]();
            });
        }
    };
        ...
        link: function($scope, $element, $attrs, formCtrl) {
            formCtrl && angular.forEach(formCtrl, function(item) {
                ...
                if (angular.isObject(item) && item.$name) {
                    ...
                    item.$element = control, control.on("focusout", checkControl), control.on("keyup", function() {
                        item.$error.showError && checkControl();
                    });
                }
            });
        }
    ...
    var inputCover = new function() {
        function createCover(element, text, className) {
            ...
            wrapper.style.position = "relative", wrapper.setAttribute("data-type", "wrapper"), 
            element.parentNode.insertBefore(wrapper, element), wrapper.appendChild(element);
            var valueText = document.createElement("span");
            valueText.style.position = "absolute", valueText.style.left = "-9999px", copyFontStyle(element, valueText), 
            valueText.innerHTML = element.value + "&nbsp;&nbsp;", document.body.appendChild(valueText);
            var cover = document.createElement("div"), offset = valueText.clientWidth;
            document.body.removeChild(valueText), element.clientWidth < offset || (cover.style.position = "absolute", 
            cover.style.left = offset + "px", cover.style.top = 0, cover.style.overflow = "hidden", 
            cover.style.cursor = "text", cloneMetrics(element, cover, offset), className && (cover.className = className), 
            cover.innerHTML = text, cover.onclick = function() {
                element.focus();
        ...
        function cloneMetrics(sourse, destination, offset) {
            var computedStyle = window.getComputedStyle ? getComputedStyle(sourse, "") : sourse.currentStyle, width = parseInt(computedStyle.width, 10);
            isNaN(width) && (width = sourse.clientWidth), destination.style.mozBoxSizing = computedStyle.mozBoxSizing, 
            destination.style.webkitBoxSizing = computedStyle.webkitBoxSizing, destination.style.boxSizing = computedStyle.boxSizing, 
            destination.style.width = width - offset + "px", destination.style.height = computedStyle.height, 
            destination.style.lineHeight = computedStyle.height, destination.style.marginLeft = computedStyle.marginLeft, 
            destination.style.marginRight = computedStyle.marginRight, destination.style.marginTop = computedStyle.marginTop, 
            destination.style.marginBottom = computedStyle.marginBottom, destination.style.borderLeftWidth = computedStyle.borderLeftWidth, 
            destination.style.borderRightWidth = computedStyle.borderRightWidth, destination.style.borderTopWidth = computedStyle.borderTopWidth, 
            destination.style.borderBottomWidth = computedStyle.borderBottomWidth, destination.style.paddingLeft = computedStyle.paddingLeft, 
            destination.style.paddingRight = computedStyle.paddingRight, destination.style.paddingTop = computedStyle.paddingTop, 
        ...
        function copyFontStyle(sourse, destination) {
            ...
            destination.style.fontFamily = computedStyle.fontFamily, destination.style.fontSize = computedStyle.fontSize, 
            destination.style.fontStyle = computedStyle.fontStyle;
        }
        function removeCover(element) {
            var wrapper = element.parentNode;
            if ("wrapper" == wrapper.getAttribute("data-type")) {
                var parent = wrapper.parentNode;
                parent.insertBefore(element, wrapper), parent.removeChild(wrapper), element.focus();
            }
        }
        function $createCover($element, text, className) {
    ...
    return {
        ...
        link: function(scope, element) {
            function onSelectCity(suggestion) {
                if (scope.isNoneRegion = !1, scope.Adress.City = suggestion.data.City, scope.Adress.Town = suggestion.data.Town, 
                scope.Adress.Rajon = suggestion.data.Rajon, scope.Adress.Region = suggestion.data.Region, 
                0 != suggestion.data.City.CodeType && (scope.Adress.CityTownName = suggestion.data.City.Name), 
                0 != suggestion.data.Town.CodeType && (scope.Adress.CityTownName = suggestion.data.Town.Name), 
                scope.$apply(), inputCover.$remove(cityInput), 77 != suggestion.data.Region.Code && 78 != suggestion.data.Region.Code) {
                    var cover = [];
                    0 != suggestion.data.Region.CodeType && suggestion.data.Region.Name && cover.push(suggestion.data.Region.DisplayName || ""), 
                    0 != suggestion.data.Rajon.CodeType && suggestion.data.Rajon.Name && cover.push(suggestion.data.Rajon.Name + " " + suggestion.data.Rajon.NameType), 
                    inputCover.$create(cityInput, cover.join(", "), "text lightgray");
                }
            }
            function setEmptyCodes() {
                inputCover.$remove(cityInput), scope.Adress.City.Code = scope.Adress.City.CodeType = scope.Adress.Town.Code = scope.Adress.Town.CodeType = scope.Adress.Rajon.Code = scope.Adress.Rajon.CodeType = scope.Adress.Region.Code = scope.Adress.Region.CodeType = scope.Adress.Street.CodeType = 0, 
                scope.Adress.City.Name = null, scope.isNoneRegion || (scope.Adress.Rajon.Name = scope.Adress.Region.Name = null), 
                scope.isNoneRegion = !0, scope.$apply();
            }
            var cityInput = $(".js-kladr-city-input", element), cityInputParent = cityInput.parent(".input-block"), streetInput = $(".js-kladr-street-input", element), streetInputParent = streetInput.parent(".input-block"), rajonInput = $(".js-kladr-rajon-input", element), rajonInputParent = rajonInput.parent(".input-block"), regionInput = $(".js-kladr-region-input", element), regionInputParent = regionInput.parent(".input-block"), inputCollection = cityInput.add(streetInput).add(rajonInput).add(regionInput), inputParentCollection = inputCollection.parent(".input-block"), domInput = $(".js-kladr-dom-input", element);
            inputCollection.on("focus", function() {
                inputParentCollection.css("position", "static"), $(this).parent(".input-block").css("position", "relative");
                ...
                onSelect: function(suggestion) {
                    onSelectCity(suggestion), streetInput.focus();
                },
                onSearchComplete: function(query, suggestions) {
                    0 == suggestions.length ? setEmptyCodes() : 1 == suggestions.length ? cityInput.off("focusout.selectOneItem").one("focusout.selectOneItem", function() {
                        onSelectCity(suggestions[0]);
                    }) : scope.$apply(scope.isNoneRegion = !1);
                },
                ...
                onSelect: function(suggestion) {
                    scope.Adress.Street = suggestion.data.Street, scope.Adress.Street.Name = suggestion.value, 
                    scope.$apply(), domInput.focus();
                },
                onSearchStart: function() {},
                onSearchComplete: function(query, suggestions) {
                    0 == suggestions.length ? (scope.Adress.Street.Code = scope.Adress.Street.CodeType = 0, 
                    scope.Adress.Street.NameType = null, scope.$apply()) : 1 == suggestions.length && streetInput.off("focusout.selectOneItem").one("focusout.selectOneItem", function() {
                        scope.Adress.Street = suggestions[0].data.Street, scope.$apply();
                    });
                }
                ...
                onSelect: function(suggestion) {
                    scope.Adress.Rajon = suggestion.data.Rajon, scope.Adress.Region = suggestion.data.Region, 
                    scope.$apply(), streetInput.focus();
                },
                onSearchStart: function() {},
                onSearchComplete: function(query, suggestions) {
                    0 == suggestions.length ? (scope.Adress.Rajon.Code = scope.Adress.Rajon.CodeType = 0, 
                    scope.Adress.Rajon.NameType = null, scope.$apply()) : 1 == suggestions.length && rajonInput.off("focusout.selectOneItem").one("focusout.selectOneItem", function() {
                        scope.Adress.Rajon = suggestions[0].data.Rajon, scope.Adress.Region = suggestions[0].data.Region, 
                        scope.$apply();
                    });
                }
                ...
                onSelect: function(suggestion) {
                    scope.Adress.Rajon = suggestion.data.Rajon, scope.Adress.Region = suggestion.data.Region, 
                    scope.$apply(), rajonInput.focus();
                },
                onSearchStart: function() {},
                onSearchComplete: function(query, suggestions) {
                    0 == suggestions.length ? (scope.Adress.Region.Code = scope.Adress.Region.CodeType = 0, 
                    scope.Adress.Region.NameType = null, scope.$apply()) : 1 == suggestions.length && regionInput.off("focusout.selectOneItem").one("focusout.selectOneItem", function() {
                        scope.Adress.Rajon = suggestions[0].data.Rajon, scope.Adress.Region = suggestions[0].data.Region, 
                        scope.$apply();
                    });
                }
            }), scope.$watch("Adress", function(newValue) {
                if (newValue) {
                    var options = {
                        params: {
                    ...
                    streetInput.autocomplete("setOptions", options), rajonInput.autocomplete("setOptions", options), 
                    newValue.CityTownName || (scope.isNoneRegion = !1);
                }
            }, !0), inputCollection.off("focus.autocomplete"), scope.$on("$destroy", function() {
                cityInput.autocomplete("dispose"), streetInput.autocomplete("dispose"), regionInput.autocomplete("dispose"), 
        ...
        link: function(scope, element, attrs) {
            angular.element(element).data("popupCaller", !0), function(attrs) {
                element.bind("click", function() {
                    ...
                    layers.layersList.length > 1 && !layers.isDownInTree(this) || (1 != layers.layersList.length || angular.element(layers.layersList[0]).hasClass("popup") || (angular.element(layers.layersList[0]).scope().visible = !1, 
                    layers.layersList.pop()), angular.forEach(layers.layersList, function(item) {
                        angular.element(item).hasClass("popup") && (angular.element(item).scope().visible = !1);
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            function touchSection() {
                $scope.currentSection = $parentScope.currentSection.anchor, $scope.currentSection == $attrs.anchor && "function" == typeof $scope[$attrs.onOpen] && $scope[$attrs.onOpen]();
            }
            var $parent = angular.element($element).parent(), $parentScope = $parent.scope();
            $scope.section = $attrs.anchor, $parentScope.$watch("[currentSection, visible]", function() {
                $parentScope.visible && touchSection();
            }, !0);
        }
    };
    ...
    return {
        ...
        link: function(scope, element) {
            ...
            fields.length < 2 || fields.each(function(index, item) {
                ...
                maxlength && $(item).on("keyup", function(event) {
                    var key = event.keyCode;
                    39 != key && 37 != key && maxlength == $(this).val().length && fields[index + 1] && $timeout(function() {
                        $caretPosition.set(fields[index + 1], 0);
                    }, 1);
                });
            });
        ...
        controller: function($scope) {
            ...
            this.select = function(item) {
                angular.forEach(items, function(item) {
                    item.selected = !1;
                }), item.selected = !0, angular.isDefined(item.modelValue) ? $scope.modelValue = item.modelValue : item.handler && item.handler();
                ...
                angular.forEach(items, function(item) {
                    ...
                    item.modelValue === $scope.modelValue && (currentItem = item);
                }), currentItem && (currentItem.selected = !0, currentItem.handler && currentItem.handler());
            }, this.addItem = function(item, element, isDefault) {
                0 == items.length && this.select(item), isDefault && this.select(item), items.push(item);
            };
        },
        link: function($scope, $element, $attrs) {
    ...
    return {
        ...
        link: function(scope, element, attrs, switcherCtrl) {
            switcherCtrl.addItem(scope, attrs.isDefault), scope.select = switcherCtrl.select;
        },
    ...
    return {
        ...
        controller: function($scope) {
            ...
            $scope.select = function(tab) {
                angular.forEach(tabs, function(tab) {
                    tab.selected = !1;
                }), tab.selected = !0, tab.handler && tab.handler();
        ...
        link: function($scope, $element) {
            if ($element.hasClass("nav-tabs")) {
                var boxWidth = $element.width(), elementsWidthArray = [], elementsWidth = 0;
                $timeout(function() {
                    function calculateTabsWidth() {
                        if ($scope.hiddenList = [], elementsWidth = 0, boxWidth = $element.width(), angular.forEach(elementsWidthArray, function(item) {
                            item.hidden = !1, elementsWidth += item.width;
                        }), elementsWidth += btnMoreWidth, elementsWidth > boxWidth) {
                            var i = 1;
                            for (elementsWidthArray[length - i].hidden = !0, i++, elementsWidthArray[length - i].hidden = !0, 
                            elementsWidth = 0, angular.forEach(elementsWidthArray, function(item) {
                                item.hidden || (elementsWidth += item.width);
                            }), elementsWidth += btnMoreWidth; elementsWidth > boxWidth; ) elementsWidth = 0, 
                            elementsWidthArray[length - i++].hidden = !0, angular.forEach(elementsWidthArray, function(item) {
                                item.hidden || (elementsWidth += item.width);
                            }), elementsWidth += btnMoreWidth;
                        }
                        angular.forEach($scope.tabs, function(item, index) {
                            item.hidden = elementsWidthArray[index].hidden, item.hidden && $scope.hiddenList.push(item);
                        });
                    }
                    ...
                    var length = elementsWidthArray.length, btnMoreWidth = $(".fake-btn", $element).outerWidth();
                    calculateTabsWidth(), $(window).on("resize", function() {
                        calculateTabsWidth(), $scope.$apply();
    ...
    function updateLayers(element) {
        _layers.length > 0 && (angular.element(element).hasClass("pop-on-click") || !angular.element(element).data("popupCaller") && (!isUpInTree(element) || !isDownInTree(element, angular.element(_layers[_layers.length - 1]))) || angular.element(element).data("popupCaller") && !isDownInTree(angular.element(element).data("targetPopup"))) && popLastLayer(), 
        updateScrollStatus();
    }
    ...
    function popLastLayer() {
        ...
        topLayerScope.visible = !1, _layers.pop(), topLayerScope.$apply(), $topLayer = angular.element(_layers[_layers.length - 1]), 
        topLayerScope = $topLayer.scope(), $topLayer.hasClass("popup") && (topLayerScope.visible = !0, 
        topLayerScope.$apply()), updateScrollStatus();
    }
    function updateScrollStatus() {
        var bodyScope = angular.element($document[0].body).scope(), noScroll = !1;
        angular.forEach(_layers, function(layer) {
            angular.element(layer).hasClass("popup") && (noScroll = !0);
        }), bodyScope.noScroll = noScroll, bodyScope.$apply();
    }
    var _layers = [];
