...
$(document).keydown(function(e) {
    ...
    return {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            var listener = function() {
                var totalValue, day = parseInt(inputDay.val()), month = parseInt(inputMonth.val()), year = parseInt(inputYear.val());
                console.log(day, month, year), options.dayLimit = 2 == month && 4 == (year + "").length ? new Date(year, month, 0).getDate() : month ? options.daysInMonth[month - 1] : 31, 
                ...
                totalValue = day + "." + month + "." + year, scope.$apply(function() {
            ...
            inputs.on("focusout", function() {
                var totalValue, day = parseInt(inputDay.val()), month = parseInt(inputMonth.val()), year = parseInt(inputYear.val());
                0 === day && inputDay.val("1"), 0 === month && inputMonth.val("1"), totalValue = day + "." + month + "." + year, 
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            $scope.visible = !1, $scope.caller = $sce.trustAsHtml($attrs.caller), $element.bind("click", function() {
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            function loadContent(currentSection, callback) {
                currentSection.loaded || $http.get(currentSection.dynLoad).success(function(response) {
                    currentSection.loaded = !0, currentSection.content = $sce.trustAsHtml(response), 
                    "function" == typeof callback && callback(response);
                });
            }
            console.log($element), "popup-section" === $element[0].tagName.toLowerCase() || ($scope.dynamic = !0, 
            $scope.loaded = !1, $scope.$watch("visible", function(newVal) {
                $scope.currentSection ? newVal && ($scope.sections[$scope.currentSection.section].loaded ? "function" == typeof $scope.dynOnOpen && $scope.dynOnOpen($scope.currentSection.section) : loadContent($scope.sections[$scope.currentSection.section], function() {
                    "function" == typeof $scope.dynOnLoad && $scope.dynOnLoad($scope.currentSection.section);
                })) : (!$scope.loaded && newVal && $http.get($attrs.dynLoad).success(function(response) {
                    $scope.loaded = !0, $scope.content = $sce.trustAsHtml(response), "function" == typeof $scope.dynOnLoad && $scope.dynOnLoad();
                }), $scope.loaded && newVal && "function" == typeof $scope.dynOnOpen && $scope.dynOnOpen());
            }), $scope.$watch("currentSection", function(newVal) {
                $scope.visible && newVal && loadContent($scope.sections[newVal.section]), $scope.visible && newVal && !$scope.sections[newVal.section].loaded && "function" == typeof $scope.dynOnLoad && $scope.dynOnLoad(newVal.section), 
                $scope.visible && newVal && $scope.sections[newVal.section].loaded && "function" == typeof $scope.dynOnOpen && $scope.dynOnOpen(newVal.section);
            }));
        }
    };
    ...
    return {
        ...
        scope: !0,
        controller: function($scope, $element, $attrs) {
            if ($scope.visible = !1, $element.bind("click", function(e) {
    ...
    return {
        ...
        controller: function($scope, $element, $attrs) {
            ...
            $attrs.dynLoad && ($parentScope.sections[$attrs.target].dynLoad = $attrs.dynLoad);
        }
    };
    ...
    function updateLayers(element) {
        (_layers.length > 0 && !isUpInTree(element) && !angular.element(element).hasClass("pop-on-click") && !angular.element(element).data("popupCaller") || _layers.length > 0 && angular.element(element).data("popupCaller") && !isDownInTree(angular.element(element).data("targetPopup"))) && popLastLayer();
    }
