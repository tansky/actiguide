...
$(document).keydown(function(e) {
    ...
    Autocomplete.utils = utils, $.Autocomplete = Autocomplete, Autocomplete.formatResult = function(suggestion, currentValue) {
        ...
        suggest: function() {
            if (document.activeElement.name === this.element.name) {
                ...
                if (options.triggerSelectOnValidInput && (index = that.findSuggestionIndex(value), 
                -1 !== index)) return void that.select(index);
    ...
    CoreUISelect.prototype.init = function() {
        return $.browser.operamini ? this : (this.buildUI(), this.hideDomSelect(), this.domSelect.is(":disabled") ? (this.select.addClass("disable"), 
        ...
        this.dropdownItem.filter(":first-child").addClass("first-item"), this.dropdownItem.filter(":last-child").addClass("last-item"), 
...
}), actiGuide.mainModule.directive("grid", function() {}), actiGuide.mainModule.directive("hint", function($timeout) {
    return {
        restrict: "A",
        priority: 1e3,
        link: function($scope, $element, $attrs) {
    ...
    return {
        ...
        link: function($scope, $element) {
            if ($element.hasClass("nav-tabs")) {
                ...
                console.log($scope.tabs), $timeout(function() {
                    if ($("button", $element).each(function(index, item) {
                    ...
                    }), elementsWidth > boxWidth) {
                        var length = elementsWidthArray.length;
                        elementsWidthArray[length - 1].hidden = !0, elementsWidthArray[length - 2].hidden = !0, 
                        elementsWidth = 0, angular.forEach(elementsWidthArray, function(item) {
                            item.hidden || (elementsWidth += item.width);
                        }), console.log($(".dropdown", $element)), console.log(boxWidth, elementsWidth, elementsWidthArray), 
